...
'''Create an estimate of the self-interaction parameter useful for interaction
areal interpolation.

The script can either print a single parameter value for the whole system
...
def read_neigh_file(path):
    df = pd.read_csv(path, sep=';')
    return list(df.itertuples(index=False, name=None))


def calculate_etas(inter_df, neigh):
    ...
    for fromid, toid in neigh:
        ...
        # etas.append((fromid, toid, eta, fromto + tofrom))
    if n_clipped:
